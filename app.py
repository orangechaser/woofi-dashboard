...
try:
    ...
    if not data.empty:
        ...
        with c1:
            fig_v = go.Figure()
            fig_v.add_trace(go.Scatter(x=data['date_range'], y=data['swap_vol'], name='Swap Vol', line=dict(color='#00FFA3')))
            fig_v.add_trace(go.Scatter(x=data['date_range'], y=data['pro_vol'], name='Pro Vol', line=dict(color='#00E0FF')))
            fig_v.update_layout(title="Weekly Volume ($)", hovermode="x unified", legend=dict(orientation="h", y=1.1))
            st.plotly_chart(fig_v, use_container_width=True)

        with c2:
            fig_r = go.Figure()
            fig_r.add_trace(go.Scatter(x=data['date_range'], y=data['swap_rev'], name='Swap Rev', line=dict(color='#FF4B4B')))
            fig_r.add_trace(go.Scatter(x=data['date_range'], y=data['pro_rev'], name='Pro Rev', line=dict(color='#FFAA00')))
            fig_r.add_trace(go.Scatter(x=data['date_range'], y=data['kronos_rev'], name='Kronos Rev', line=dict(dash='dot', color='#AA00FF')))
        ...
        if not rank_df.empty:
            fig_rank = go.Figure()
            fig_rank.add_trace(go.Scatter(x=rank_df['date_range'], y=rank_df['rank'], mode='lines+markers', line=dict(color='white')))
            fig_rank.update_layout(yaxis=dict(autorange="reversed", title="Rank"), height=300)
